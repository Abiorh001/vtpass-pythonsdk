...
class DataSubscription(VtPassPythonSDK):
    def purchase_data_susbscription(self, url: str, data_sub_schema: DataSubscriptionSchema):
        """
        ...
        try:
            purchase_data_subscription_url = f"{url}/pay"
            response = requests.post(purchase_data_subscription_url, headers=headers, data=json.dumps(data))
            ...
            if "code" in result and result["code"] != "000":
                ...
                logging.debug(f"Data Subscription purchased successfully for {data_sub_schema.phone}")
                if jr == "True":
                    return result
                else:
    ...
    def verify_smile_email(self, url: str, verify_smile_schema: VerifySmileEmailSchema):
        ...
        try:
            response = requests.post(verify_smile_email_url, headers=headers, data=json.dumps(data))
            response.raise_for_status()
            result = response.json()
